 ...
 * 02/12/2018|mnutsch|KACE:20683 - Added a section for "Resample" test types.
 * 02/16/2018|mnutsch|KACE:xxxxx - Fixed a bug related to an error message about $resampleTestsSubtotal being undefined.
 * 
 * **************************************************************************************************************************************** */
...
function getPerfCycles($plantId, $labTech, $startDate, $endDate)
    {
  $sql = mysqli_query(connectToMySQLQC(), "CALL sp_gb_qc_PerformanceCyclesGetByLabTech(" . $plantId . "," . $labTech . ",'" . $startDate . "','" . $endDate . "');");
    $rows = array();
...
$cyclesDownloadArray[0] = "Plant,Lab Tech,Test,Avg Hrs (Tests),Retests,Misc Tests,Calibration Tests,Repeatability Tests,Resample Tests";
...
<table class='table table-bordered table-striped table-responsive'>
  <thead style="font-weight:bold">
    <tr><th>Plant</th><th>Lab Tech</th><th>Tests</th><th>Avg Hrs (Tests)</th><th>Retests</th><th>Misc Tests</th><th>Calibration Tests</th><th>Repeatability Tests</th><th>Resample Tests</th><th>Charts</th></tr>
  </thead>
  <tbody>
  ...
  <?php
    ...
    for ($i = 0; $i < count($cyclesArray); $i++) 
    {
      ...
      if($cyclesArray[$i]->vars['plant_id'] != $previousPlant)
      {
        if($subtotalCount != 0)
        {
          ...
          echo("<tr style='font-weight:bold; background-color: var(--vprop_blue_light);'>
                  ...
                  <td>" . $resampleTestsSubtotal . "</td>
                  <td> </td>
                </tr>");

          $cyclesDownloadArray[$cyclesDownloadCount] = "Subtotal,," . $testsSubtotal . "," . $avgHoursSubAvg . "," . $retestsSubtotal . "," . $miscTestSubtotal . "," . $calibrationTestsSubtotal . "," . $repeatabilityTestsSubtotal . "," . $resampleTestsSubtotal;
          $cyclesDownloadCount += 1;
        }
      ...
      $resampleTestsSubtotal += $cyclesArray[$i]->vars['resample_count'];
      ...
      $resampleTestsTotal += $cyclesArray[$i]->vars['resample_count'];
      ...
      $plantHolder=$plantObject->vars["name"];
      $nameHolder=$userObjectDisplayName;
      $testHolder=$cyclesArray[$i]->vars['test_count'];
      $durationHolder=round($cyclesArray[$i]->vars['avg_hours'],2);
      $retestHolder=$cyclesArray[$i]->vars['retest_count'];
      $miscHolder=$cyclesArray[$i]->vars['misc_count'];
      $calibrationHolder=$cyclesArray[$i]->vars['calibration_count'];
      $repeatabilityHolder=$cyclesArray[$i]->vars['repeatability_count'];
      $resampleHolder=$cyclesArray[$i]->vars['resample_count'];
      ...
      echo("<tr>
              ...
              <td>" . $cyclesArray[$i]->vars['resample_count'] . "</td>
              <td> 
           
           <div class='popup' onclick='chartFunction()'>
           <form method='post' action='../../Includes/QC/performanceChart.php' >

            <input type='hidden' name='plant' value=\"$plantHolder\">
            <input type='hidden' name='techName' value='$nameHolder'>
            <input type='hidden' name='testCount' value='$testHolder'>
            <input type='hidden' name='aveDuration' value='$durationHolder'>
            <input type='hidden' name='retestCount' value='$retestHolder'>
            <input type='hidden' name='miscCount' value='$miscHolder'>
            <input type='hidden' name='calibrationCount' value='$calibrationHolder'>
            <input type='hidden' name='repeatabilityCount' value='$repeatabilityHolder'>
            <input type='hidden' name='resampleCount' value='$resampleHolder'>
            <button type='submit' name='performanceChart' value='$nameHolder' class='btn'>
            <i class='fas fa-chart-pie'></i><span class='popuptext' id='myPopup'>
            </span>
            </button> 
            </form>
           </div>
      ...
      $cyclesDownloadArray[$cyclesDownloadCount] = $plantObject->vars["name"] . "," . $userObjectDisplayName . "," . $cyclesArray[$i]->vars['test_count'] . "," . round($cyclesArray[$i]->vars['avg_hours'],2) . "," . $cyclesArray[$i]->vars['retest_count'] . "," . $cyclesArray[$i]->vars['misc_count'] . "," . $cyclesArray[$i]->vars['calibration_count'] . "," . $cyclesArray[$i]->vars['repeatability_count'] . "," . $cyclesArray[$i]->vars['resample_count'];
      $cyclesDownloadCount += 1;
  ...
  echo("<tfoot>");
    ...
    echo("<tr style='font-weight:bold; background-color: var(--vprop_blue_medium); color: #FFFFFF;'>
            ...
            <td>" . $resampleTestsTotal . "</td>
              <td> </td>
          </tr>");
    $cyclesDownloadArray[$cyclesDownloadCount] = "Total,," . $testsTotal . "," . $avgHoursTotalAvg . "," . $retestsTotal . "," . $miscTestTotal . "," . $calibrationTestsTotal . "," . $repeatabilityTestsTotal . "," . $resampleTestsTotal;
    $cyclesDownloadCount += 1;
...
?>
<input type="submit" class="btn btn-vprop-green"  class="btn btn-vprop-green" value="Export Cycles to Excel">
</form>
...
?>
<input type="submit" class="btn btn-vprop-green"  value="Export Repeatability to Excel">
</form>
...
<div style="clear: left;">
  <hr>
  <br/>
  <input type="button" value="Refresh Page" class="btn btn-vprop-blue-medium" onClick="window.location.reload()"> <br/><br/>
</div>
</div> <!-- tab content -->
...
<!--   Javascript file for popup -->
<script>
// When the user clicks on div, open the popup
function chartFunction() {

  window.open("../../Includes/QC/qcPerformance.html", "myPopup", 'width=600,height=400');
  window.close();
}
</script>
