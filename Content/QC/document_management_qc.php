...
$departmentId = 4;
$departmentName = 'QC';
$lwrDeptName = strtolower($departmentName);
...
//echo (print_r($userPermissionsArray['vista']));
  ...
  if($departmentId !== 4)
  {
  $permissionLevel = $userPermissionsArray['vista']['granbury'][$lwrDeptName];
  }
  else
    {
      ...
      if(isset($userPermissionsArray['vista']['granbury']['qc']))
        {
          $permissionLevelGb = $userPermissionsArray['vista']['granbury']['qc'];
        }
      if(isset($userPermissionsArray['vista']['tolar']['qc']))
        {
          $permissionLevelTl = $userPermissionsArray['vista']['tolar']['qc'];
        }
      if(isset($userPermissionsArray['vista']['west_texas']['qc']))
        {
          $permissionLevelWt = $userPermissionsArray['vista']['west_texas']['qc'];
        }
...
window.$global = {
  ...
  maxPerm:<?php echo $permissionLevel; ?>};
...
formData = JSON.stringify(formData);
  ...
  $("#upload-button").click(function() {
    ...
    $('#drop-area').data('dmUploader').settings.extraData = {
        ...
        deptName: $global.departmentName
    };
  ...
  $('#edit-file-submit-btn').click(function(){ 
    ...
    if (description == "") {
            $('#failure-msg-file-edit').html("Description cannot be blank").fadeIn('slow')
            $('#failure-msg-file-edit').delay(3000).fadeOut('slow');
            return;
        }
    if(parseInt(category) === -1){
        $('#failure-msg-file-edit').html("Please Select a category").fadeIn('slow')
        $('#failure-msg-file-edit').delay(3000).fadeOut('slow');
        return;
    }
      $.ajax
          ...
          data:
                  {
                    ...
                    department:department,                    
                    user_id:$global.userId,
                    dept_name:$global.departmentName
...
//    $('#file-replace').modal('hide');    
    ...
    formData.append('deptName', $global.departmentName);
    $.ajax
