<?php
/* * *****************************************************************************************************************************************
 * File Name: wt_qcsamplegroupadd.php
 * Project: Silicore
...
if($isValidationError == 1)
{
  ...
  //redirect the user 
  header('Location: ../../Controls/QC/wt_samplegroupadd.php?validationStatus=fail');
}
...
try
{
  ...
  $sql = "CALL sp_wt_qc_ShiftsGetBySiteAndDate(" . $plantId . ",'" . $dt . "');";
...
if($siteId != 50) //siteID 10 = Granbury, 50 = Tolar
{
  ...
  //$sendExternalQCSampleNotificationResult = sendExternalQCSampleNotification(1, $siteObject->vars['description'], $sampleIDs, "West Texas");
}
...
echo("<script>window.location.replace('../../Controls/QC/wt_samples.php?view=summary&completionStatus=Incomplete');</script>");
