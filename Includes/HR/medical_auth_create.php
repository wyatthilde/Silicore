...
if(substr($appEmpCode, 0,3) == '001') { //employee prefix
   $empCode = substr($appEmpCode, 3);
    ...
    foreach($applicant as $attr) {
        ...
        $division = $attr->hr_division_id;
        $location = $attr->hr_site_id;
        $createDate = $attr->create_date;
    }
}
...
foreach($tests as $test){
    $query = 'sp_hr_TestNameGet("'. $test->hr_lab_test_id . '")';
    $mpdf->WriteHTML('<tr><td style="border:1px solid;">' . json_decode($db->get($query))[0]->description . '</td></tr>');
}
$mpdf->WriteHTML('</table>');

$filename = $requestId . '-' . $clinicId . '.pdf';
$target = '../../Files/HR/Medical/Authorizations/' . $filename;
$fileQuery = 'sp_hr_MedicalAuthFilePathInsert("' . $requestId . '","' . $target . '")';
$db->insert($fileQuery);
