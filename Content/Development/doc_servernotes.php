...
<ul>
  ...
  <li>Copy working codebase from dev to test:
    <ul>
      <li>Dump sprocs with mysqldump -u root -p silicore_site -n -d -t --routines > /var/www/data/DeployStoredProcYYYMMDD.sql </li>
      <li>Use Filezilla to pull latest code tree and sproc dump to the staging station from the development server. 
  ...
  <li>Stop the Apache server completely until set-up is complete.</li>
  <li>Back up test database from the command line on the production server with the following command (as root):
    <ul>
      <li>mysqldump -u root -p silicore_site --routines &gt; /var/www/data/silicore_site_YYYYMMDD_buildXXX_deploy.sql</li>
    </ul>
  </li>
  ...
  <li>
    Archive any existing php batch jobs in /var/www/cron/silicore/site/ (into the contained Archive folder) and copy over the latest version
    (if changes have been made) from the code tree. This will need to be done from the command line (sudo) since root is the owner of the cron
    branch.
  </li>
...
<br /><br />
Task Ticket Todo List:
<pre>
TODO (dev to test deploy):
* [] Commit final build on the dev server [hg addremove hg commit -m \"Build xxx deploy\"]
* [] Copy working codebase from dev to test
* [] Back up test database [mysqldump -u root -p silicore_site --routines &gt; /var/www/data/silicore_site_YYYYMMDD_buildXXX_deploy.sql]
* [] Execute database deploy scripts 
* [] Archive old (IMPORTANT!) and copy new batch scripts from code tree to /var/www/cron/silicore/site as needed.
* [] Delete symlink, create new, point to new build directory
* [] Restart Apache 
* [] Have devs verify that their code is working
* [] Send email to team(s) requesting user testing
* [] Update /Includes/pagevariables.php
* [] Test thoroughly
</pre>
<br /><br />
...
<pre>
TODO (test to live deploy):
* [] Send email to team(s) with arranged downtime schedule
* [] Copy working codebase from test to production
* [] Back up production database 
* [] Execute database deploy scripts 
* [] Archive old (IMPORTANT!) and copy new batch scripts from code tree to /var/www/cron/silicore/site as needed.
* [] Delete the existing symlinked folder 
* [] Create a new symlink
* [] Restart Apache
* [] Test/verify good build and full site functionality
* [] Have devs verify that their code is working
* [] Send email to team(s) announcing completion of Silicore v.X.X (BuildXXX)
</pre>
