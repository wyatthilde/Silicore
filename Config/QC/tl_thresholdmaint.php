<?php
/* * *****************************************************************************************************************************************
 * File Name: tl_thresholdmaint.php
 * Project: Silicore
 ...
 * 07/19/2018|zthale|KACE:xxxxx - gb_thesholdmaint.php is functioning as expected, but I noticed during my restyling that this file and wt_thresholdmaint.php aren't inserting/updating properly.
 * **************************************************************************************************************************************** */
...
<h1>Threshold Maintenance (Tolar)</h1><br /><br />

<div class="container">
      <form name="addThresholdForm" id="addThresholdForm" action="../../Includes/QC/tl_thresholdadd.php" method="get">
  ...
  <table class="table table-sm table-striped table-hover dt-responsive" id='qcThresholdMaintenanceTable' name='qcThresholdMaintenanceTable' style="width: 100%;"> 
      ...
      <?php
            for($i = 0; $i < count($qcThresholdArray); $i++)
            {
              ...
              $editURL = "../../Controls/QC/tl_thresholdedit.php?thresholdID=" . urlencode($qcThresholdArray[$i]->vars['id']) . "&thresholdScreen=" . urlencode($qcThresholdArray[$i]->vars['screen']) . "&thresholdLocation=" . urlencode($qcThresholdArray[$i]->vars['location_id']) . "&thresholdLow=" . urlencode($qcThresholdArray[$i]->vars['low_threshold']) . "&thresholdHigh=" . urlencode($qcThresholdArray[$i]->vars['high_threshold']) . "&thresholdIsActive=0";            

              //create a URL that would void this sample
              $voidURL = "../../Includes/QC/tl_thresholdupdate.php?thresholdID=" . urlencode($qcThresholdArray[$i]->vars['id']) . "&thresholdScreen=" . urlencode($qcThresholdArray[$i]->vars['screen']) . "&thresholdLocation=" . urlencode($qcThresholdArray[$i]->vars['location_id']) . "&thresholdLow=" . urlencode($qcThresholdArray[$i]->vars['low_threshold']) . "&thresholdHigh=" . urlencode($qcThresholdArray[$i]->vars['high_threshold']) . "&thresholdIsActive=0";            
