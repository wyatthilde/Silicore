...
if($main_site_id == 10)
{
    $sql = "CALL sp_gb_qc_LocationsUpdate($id, '$description', $main_site_id, $main_plant_id, $is_split_sample_only, $order, $isActive, $user_id);"; // Updates edited location from gb_qc_locations table.
  ...
  {
    $sql = "CALL sp_tl_qc_LocationsUpdate($id, '$description', $main_site_id, $main_plant_id, $is_split_sample_only, $order, $isActive, $user_id);"; // Updates edited location from tl_qc_locations table.
    $result = $mySQLConnection->query($sql);

    disconnectFromMySQLQC($mySQLConnection); // Disconnect from the database.

    echo "<script type=\"text/javascript\">window.location = \"../../Controls/QC/tl_managelocations.php\";</script>"; // Using JS, because output is already sent in header.php...
  }
