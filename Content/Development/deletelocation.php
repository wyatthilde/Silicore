 ...
 * Project: smashbox
 * Description: program to delete qc locations
 * Notes:
 * =========================================================================================================================================
 * Change Log ([08/02/2018]|[Developer]|[Task Ticket] - [Description]
 * =========================================================================================================================================
 * 08/02/2018|ktaylor|KACE:24650 - Initial creation
 * 
 * **************************************************************************************************************************************** */
...
function getLocations()
{
  ...
  try
  {
    ...
    $sql = "CALL sp_qc_AllSitesLocationsGet();"; // Stored procedure method, which gets current locations to display in table.
    $result =  $mySQLConnection->query($sql);
...
<html>
	<head>
        ...
        <script>  
            // This script will execute the getplantbysite.php file, whenever the site select box changes. getplantbysite.php filters the plant select box with appropriate plants based off of the selected site id.
            $(document).ready(function() {
              $("#siteSelect").change(function() {
                $.get('../../Content/QC/getplantbysite.php?siteSelect=' + $(this).val(), function(data) {
                  $("#plantSelect").html(data);
                }); 
              });
            });
        </script>
                                  ...
                                  foreach ($locationObjectArray as $locationObject) 
                                  {
                                    ...
                                    if($locationObject->vars['active_status'] == 0)
                                    {
                                     ...
                                     else if($locationObject->vars['active_status'] == 1)
                                     {
                                      ...
                                      $locationID = $locationObject->vars['id'];
                                      $site = $locationObject->vars['site'] ;
                                     
                                      
                                      $voidURL = "../../Includes/Development/deletelocationvoid.php?locationID=" . urlencode($locationID). "&Site=" . urlencode($locationObject->vars['site']) ;

   
                                     echo ("<td>
                                     <a href= ' " . $voidURL . " ' onclick=\"return confirm('Are you sure you want to delete location - (Id #: " . $locationID . ")?')\"><!--Remove--><i class='fa fa-trash' style='font-size:20px;color:red'></i></a>
                                     </td></tr>");
