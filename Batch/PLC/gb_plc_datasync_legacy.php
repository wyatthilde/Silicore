<?php
/*******************************************************************************************************************************************
 * File Name: gb_plc_datasync_legacy.sql
 * Project: Silicore
 * Description: This script is designed to pull the Granbury PLC data and place it into a schema that mirrors the table structure that is
 *              used by Backoffice. 
 * Notes: This script will only be used long enough to replace it with the new PLC data script that directly mirrors the table schema in 
 *        VistaSQL1.DataForTransfer.
 *        The tables used in Backoffice are the following:
 *          prod_auto_plant_analog_data;
 *          prod_auto_plant_analog_data_dryer_temp;
 *          prod_auto_plant_analog_tags;
 *          prod_auto_plant_downtime;
 *          prod_auto_plant_idletime;
 *          prod_auto_plant_production;
 *          prod_auto_plant_production_rt;
 *          prod_auto_plant_runtime;
...
// kkuehn: create sendErrorEmail() wrapper to send the messages throughout the script
function sendErrorEmail()
{
  echo("into sendErrorEmail()");
}

//set batch variable to bypass user authentication 
$isBatch = 1;

//connect to dw mysql database
//include('../init.php');

// kkuehn: connect to silicore-site db (MySQL, local silicore server)


//connect to on-site mssql database
//include('../init-vpn-tunnel.php');

// kkuehn: connect to Granbury VistaSQL1.DataForTranfer (MSSQL over VPN)

//////////////////step 1, update the warehouse with the production information from the ESP system
// ShiftId > 9799 and ShiftId < 9857
$reportSql = mssql_query('
select 
...
order by ShiftId
') or die(sendErrorEmail(mssql_get_last_message()));

while($report = mssql_fetch_array($reportSql)) {
	$productId = 0;
	$tagId = 'null';
	$tag = $report['TotalDesc'];
	
	$reportSql2 = mysqli_query($mysqliConn, 'select id from prod_auto_plant_analog_tags where tag = "'.$tag.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
	while($report2 = mysqli_fetch_array($reportSql2)){
		$tagId = '"'.$report2['id'].'"';
	}
	
	$reportSql2 = mysqli_query($mysqliConn, 'select id from prod_products where tag = "'.$tag.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
	while($report2 = mysqli_fetch_array($reportSql2)){
	...
	//if ($shiftId < 1831 || $shiftId > 1838) {
		$reportSql2 = mysqli_query($mysqliConn, 'select id from prod_auto_plant_production where shift_id = "'.$shiftId.'" and tag_id = '.$tagId.' and product_id = "'.$productId.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
...
$reportSql = mssql_query('
select 
	Id, RtId, ShiftId, Runtime, RuntimeDescription, Tag
from 
	Runtimes
where 
	ShiftId >= (select max(ShiftId) - 32 from Runtimes)
order by ShiftId
') or die(sendErrorEmail(mssql_get_last_message()));
...
while($report = mssql_fetch_array($reportSql)) {
	...
	//insert the record
	mysqli_query($mysqliConn, 'insert into prod_auto_plant_runtime (shift_id, duration_minutes, duration, device, tag_id, tag, create_dt) values ("'.$shiftId.'", "'.$durationMinutes.'", "'.$duration.'", "'.$device.'", '.$tagId.', "'.$tag.'", "'.$currentDt.'")
				 ON DUPLICATE KEY UPDATE shift_id = "'.$shiftId.'"') or die(sendErrorEmail("line 89 " . mysqli_error($mysqliConn)));
}
//////////////////end step 2
...
while($report = mssql_fetch_array($reportSql)) {
	...
	$id = $report['DtId'];
	$shiftId = $report['ShiftId'];
	$endDt = date("Y-m-d H:i:s", strtotime($report['DtEnd']));
	$durationMinutes = $report['DtAmount'];
	$reason = $report['DtReason'];
	$deviceName = $report['DeviceName'];
	$comment = textFilter($report['Comment']);
	...
	//insert the record
	mysqli_query($mysqliConn, 'insert into prod_auto_plant_downtime (id, shift_id, start_dt, end_dt, end_dt_short, duration_minutes, duration, reason, device_name, comments, is_scheduled) values ("'.$id.'", "'.$shiftId.'", "'.$startDt.'", "'.$endDt.'", "'.$endDtShort.'", "'.$durationMinutes.'", "'.$duration.'", "'.$reason.'", "'.$deviceName.'", "'.$comment.'", "'.$isScheduled.'")
				 ON DUPLICATE KEY UPDATE id = "'.$id.'", shift_id = "'.$shiftId.'", start_dt = "'.$startDt.'", end_dt = "'.$endDt.'", end_dt_short = "'.$endDtShort.'", duration_minutes = "'.$durationMinutes.'", duration = "'.$duration.'", reason = "'.$reason.'", device_name = "'.$deviceName.'", comments = "'.$comment.'", is_scheduled = "'.$isScheduled.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
}
//////////////////end step 3
...
while($report = mssql_fetch_array($reportSql)) {
	...
	$id = $report['ItId'];
	$shiftId = $report['ShiftId'];
	$endDt = date("Y-m-d H:i:s", strtotime($report['ItEnd']));
	...
	mysqli_query($mysqliConn, 'insert into prod_auto_plant_idletime (id, shift_id, start_dt, end_dt, end_dt_short, duration_minutes, duration, reason, comments) values ("'.$id.'", "'.$shiftId.'", "'.$startDt.'", "'.$endDt.'", "'.$endDtShort.'", "'.$durationMinutes.'", "'.$duration.'", "'.$reason.'", "'.$comment.'")
				 ON DUPLICATE KEY UPDATE id = "'.$id.'", shift_id = "'.$shiftId.'", start_dt = "'.$startDt.'", end_dt = "'.$endDt.'", end_dt_short = "'.$endDtShort.'", duration_minutes = "'.$durationMinutes.'", duration = "'.$duration.'", reason = "'.$reason.'", comments = "'.$comment.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
}
//////////////////end step 4
...
order by ShiftId
') or die(sendErrorEmail("Error! Unable to get shift information from VISTASQL1:DataForTransfer:Shift. MSSQL Error: " . mssql_get_last_message()));
...
while($report = mssql_fetch_array($reportSql)) {
	...
	$startDt = date("Y-m-d H:i:s", strtotime($report['StartDate']));
	...
	if (strlen($report['EndDate']) > 0 ) {
		...
		$endDt = date("Y-m-d H:i:s", strtotime($report['EndDate']));
	...
	$reportSql2 = mysqli_query($mysqliConn, 'select sum(duration_minutes) downtime_minutes from prod_auto_plant_downtime where shift_id = "'.$shiftId.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
	
	while($report2 = mysqli_fetch_array($reportSql2)){
		$downtime = round($report2['downtime_minutes'] / 60, 2);
	}

	$reportSql2 = mysqli_query($mysqliConn, 'select sum(duration_minutes) idletime_minutes from prod_auto_plant_idletime where shift_id = "'.$shiftId.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
	...
	//if ($shiftId < 1831 || $shiftId > 1838) {
		...
		if ($endDt != "null" && $durationMinutes <= 10) {
			mysqli_query($mysqliConn, 'update prod_auto_plant_shifts set is_removed = 1 where id = "'.$shiftId.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		}
	//}
}

//set 0 production values at 6am and 6pm for the Old Wet Plant
defaultOWPProductionData($yesterdayDate, "Day", $currentDt);
defaultOWPProductionData($yesterdayDate, "Night", $currentDt);
//////////////////end step 5
...
while($report = mssql_fetch_array($reportSql)) {
	...
	$reportSql2 = mysqli_query($mysqliConn, 'select id from prod_auto_plant_analog_tags where tag_plc = "'.$tagPlc.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
	...
	$analogDtStamp = strtotime($report['TagTimestamp']);
	...
	//			') or die(sendErrorEmail(mysqli_error($mysqliConn)));
	
	mysqli_query($mysqliConn, 'insert into prod_auto_plant_analog_data (tag_id, tag_plc, value, dt, dt_short, interval_seconds) values ('.$tagId.', "'.$tagPlc.'", "'.$analogValue.'", "'.$analogDt.'", "'.$analogDtShort.'", "'.$intervalSeconds.'") 
				ON DUPLICATE KEY UPDATE tag_id = '.$tagId.', tag_plc = "'.$tagPlc.'", value = "'.$analogValue.'", dt = "'.$analogDt.'", interval_seconds = "'.$intervalSeconds.'" 
				') or die(sendErrorEmail("line 324 " . mysqli_error($mysqliConn)));
...
if (strlen($minDt) > 0) {
	...
	while($report = mysqli_fetch_array($reportSql)) {
		...
		if ($volume > 0) {
			...
			$reportSql2 = mysqli_query($mysqliConn, 'select tons, dt dt_previous from prod_inventory_silos where dt < "'.$dt.'" and volume > 0 and site_id = "'.$siteId.'" and silo_id = "'.$siloId.'" order by dt desc limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
			...
			$reportSql2 = mysqli_query($mysqliConn, 'select sum(net_pounds) ticket_pounds from prod_tickets where dt between "'.$dtPrevious.'" and "'.$dt.'" and site_id = "'.$siteId.'" and silo_id = "'.$siloId.'" and void_status_code <> "V"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		...
		$reportSql2 = mysqli_query($mysqliConn, 'select id from prod_inventory_silos where dt = "'.$dt.'" and site_id = "'.$siteId.'" and silo_id = "'.$siloId.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		...
		//on duplicate key update id = '.$inventorySiloId.', date = "'.$date.'", time = "'.$time.'", dt = "'.$dt.'", dt_short = "'.$dtShort.'", site_id = "'.$siteId.'", silo_id = "'.$siloId.'", product_id = "'.$productId.'", volume = "'.$volume.'", tons = "'.$tons.'", loads = "'.$loads.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		mysqli_query($mysqliConn, 'insert into prod_inventory_silos (id, date, time, dt, dt_short, site_id, silo_id, product_id, volume, tons, loads, is_hour_first_reading) values ('.$inventorySiloId.', "'.$date.'", "'.$time.'", "'.$dt.'", "'.$dtShort.'", "'.$siteId.'", "'.$siloId.'", "'.$productId.'", "'.$volume.'", "'.$tons.'", "'.$loads.'", "'.$isHourFirstReading.'")
		on duplicate key update id = '.$inventorySiloId.'') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		//make sure we are reporting inventory counts from silos 3, 4, & 5 - get the latest one if not reported already
		$inventoryCheckSql = mysqli_query($mysqliConn, 'select date, time, silo_id from prod_inventory_silos where dt = "'.$dt.'" and silo_id in (3,4,5)') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		
		if (mysqli_num_rows($inventoryCheckSql) == 0) {
			//keep silo 3 invetory data in sync
			$inventoryCheckSql2 = mysqli_query($mysqliConn, 'select date, time, silo_id, product_id, tons, loads from prod_inventory_silos where dt < "'.$dt.'" and silo_id in ("3") order by date desc, time desc limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
			
			while($inventoryCheck2 = mysqli_fetch_array($inventoryCheckSql2)) {
				//$isHourFirstReading = siloInventoryIsHourFirstReading(3, $dt);
				
				mysqli_query($mysqliConn, 'insert into prod_inventory_silos (date, time, dt, dt_short, silo_id, product_id, tons, loads) values ("'.$date.'", "'.$time.'", "'.$dt.'", "'.$dtShort.'", "'.$inventoryCheck2['silo_id'].'", "'.$inventoryCheck2['product_id'].'", "'.$inventoryCheck2['tons'].'", "'.$inventoryCheck2['loads'].'")') or die(sendErrorEmail(mysqli_error($mysqliConn)));
			}
			...
			$inventoryCheckSql2 = mysqli_query($mysqliConn, 'select date, time, silo_id, product_id, tons, loads from prod_inventory_silos where dt < "'.$dt.'" and silo_id in ("4") order by date desc, time desc limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
			...
			$inventoryCheckSql2 = mysqli_query($mysqliConn, 'select date, time, silo_id, product_id, tons, loads from prod_inventory_silos where dt < "'.$dt.'" and silo_id in ("5") order by date desc, time desc limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
...
while($reportCheck = mysqli_fetch_array($checkSql)){
	...
	while($report = mysqli_fetch_array($reportSql)) {		
		...
		$reportSql2 = mysqli_query($mysqliConn, 'select tag_id, tag, description from prod_products where id = "'.$plantProductionProductId.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		...
		$reportSql2 = mysqli_query($mysqliConn, 'select p.id from prod_auto_plant_production p where p.product_id = "'.$plantProductionProductId.'" and p.shift_id = "'.$shiftId.'" and p.tag = "'.$plantProductionTag.'" limit 1') or die(sendErrorEmail(mysqli_error($mysqliConn)));
		...
		if ($avgCyclones12Stph > 0) {
			...
			while($report2 = mysqli_fetch_array($reportSql2)) {
				...
				if ($tonsRepresented > 0) {
					mysqli_query($mysqliConn, 'update prod_qc_samples set tons_represented = "'.$tonsRepresented.'", tph_represented = "'.$tphRepresented.'" where id = "'.$sampleId.'"') or die(sendErrorEmail(mysqli_error($mysqliConn)));
				}
			}
...
while($reportCheck = mysqli_fetch_array($checkSql)){
	...
	*/
	...
	mysqli_query($mysqliConn, 'insert into prod_inventory (shift_id, plant_id, product_id, production_tons, loadout_tons, tons_delta, start_tons, end_tons)
					values ("'.$shiftId.'", "'.$plantId.'", "'.$productionProductId.'", "'.$productionTons.'", "'.$loadoutTons.'", "'.$tonsDelta.'", "'.$previousTons.'", "'.$endTons.'")') or die(sendErrorEmail(mysqli_error($mysqliConn)));
}
