...
if(count($plcArray) > 0)
{
  foreach ($plcArray as $plcObject) 
  {
    ...
    if($tagObject != NULL)
    {
      echo('<input type="number" step="0.0001" max="99999" id="plc' . $objectCounter . '" name="plc[]" value="' . $tagObject->vars['value'] . '" />');           
    }
    else
    {
      echo('<input type="number" step="0.0001" max="99999" id="plc' . $objectCounter . '" name="plc[]" value="" />');
    }
