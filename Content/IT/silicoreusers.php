    ...
    <div class="card">
        ...
        <div class="card-body" id="user-card-body">
            ...
            <div class="table-responsive-xl hide" id="table-wrapper">
                <table class="table table-xl table-striped table-hover table-bordered nowrap" id='users-table' style="width: 100%;padding-bottom:25px;">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="employee-modal-title">Add Silicore User</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                ...
                <div id="new-user-content">
                    ...
                    <div class="form-group col-xl">
                        <div class="input-group">
                            <input type="text" class="form-control border-right-0" name="first_name" id="modal-first-name" placeholder="First Name" required>
                    ...
                    <div class="form-group col-xl">
                        <div class="input-group">
                            <input type="text" class="form-control border-right-0" name="last_name" id="modal-last-name" placeholder="Last Name" required>
                            <div class="input-group-append">
                                <span class="input-group-text border-left-0 bg-white"><i class="fa fw fa-user text-secondary"></i></span>
                            </div>
                        </div>
                    ...
                    <div class="form-group col-xl">
                        <div class="input-group">
                            <input type="text" class="form-control border-right-0" name="email_address" id="modal-email-address" placeholder="Email" required>
                            <div class="input-group-append">
                    ...
                    <div class="form-group col-xl">
                        <div class="input-group">
                            <select name="company" class="form-control border-right-0" id="modal-company" required>
                    ...
                    <div class="form-group col-xl">
                        <div class="input-group">
                            <select name="department" class="form-control border-right-0" id="modal-department" required>
                                <?php
                    ...
                    <div class="form-group col-xl">
                        <div class="input-group">
                            <select name="accessLevel" class="form-control border-right-0" id="modal-access-level" required>
                                <?php
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-vprop-blue-medium update-employee-btn" id="new-user-submit-btn">Submit User</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(function () {
        ...
        let $usersTable = $('#users-table').DataTable({
            dom: "<row<col-xl-12Bf>>" + "<row<col-xl-12tr>>" + "<row<col-xl-5i><col-xl-7p>>",
            ...
            columns: [
                ...
                {data: 'first_name'},
                {data: 'last_name'},
                {data: 'display_name', visible: false},
                {data: 'email'},
                {data: 'username'},
                {data: 'company'},
                {
                ...
                },
                {
                    data: null, render: function (row, meta, type, data) {
                        return '<div class="dropdown">' +
                            ...
                            '<a class="dropdown-item edit-user" href="#">Edit</a>' +
                            '<a class="dropdown-item password-reset" href="#">Reset password</a>' +
                            '</div>' +
                            '</div>';
                    }
                }
            ...
            buttons:
                [
                    {extend: 'excel', text: 'Export', className: 'users-export', title: 'Silicore Users Export ' + date},
                    {
                        text: 'New User', className: 'new-user-button', id: 'new-user-button', action: function () {
            ...
            initComplete: function () {
                ...
                $(".new-user-button").detach().appendTo('#user-card-header').removeClass().addClass('btn btn-vprop-blue form-inline float-right mr-1');
        ...
        usersTable.on('click', 'tr .edit-user', function () {
        ...
        usersTable.on('click', 'tr .password-reset', function () {
            let data = $usersTable.row($(this).closest('tr')).data();
            $.confirm({
                icon: 'fa fa-warning',
                type: 'orange',
                ...
                content: 'Are you sure you want to reset password for ' + data.first_name + ' ' + data.last_name + '?<br><small> This will send a temporary password to the users email.</small>',
                buttons: {
                    pwdReset: {
                        text: 'Reset Password',
                        ...
                        action: function () {
                            ...
                            $.ajax({
                                url: '../../Includes/IT/password_reset.php',
                                data: formData,
                                type: 'POST',
        ...
        $('#new-user-submit-btn').on('click', function () {
            var mailtest = validateNewUserFields();
            if (!mailtest) {
                return;
            }
            let formData = {};
            let newUserInputs = $('#new-user-content input, #new-user-content select');
            newUserInputs.each(function () {
                let value = $(this).val();
...
//              console.log(formData);
            $.ajax({
                url: '../../Includes/IT/newsilicoreuser.php',
                type: 'POST',
                data: JSON.stringify(formData),
                success: function (data) {
                    console.log(data);
                    let response = JSON.parse(data);
                    if (response === 1) {
                        successAlert('New user created');
                        $('#new-user-modal').modal('hide');
                    }
                    else {
                        errorAlert('Something went wrong: ' + response);
                    }
    ...
    function editUserForm(data) {
        ...
        let username = gStart + 'Username</span></div><input id="username" type="text" class="form-control rounded-left" value="' + data.username + '">' + groupEnd;
        ...
        let userForm = idInput + name + displayName + email + username + company + department + userType + lastLogin + dates + status;
    ...
    function validateNewUserFields() {
        var emailValue = $('#modal-email-address').val();
        var firstName = $("#modal-first-name").val();
        var lastName = $("#modal-last-name").val();
        var splitArray = emailValue.split('@');
        var acceptList = ["vprop.com", "maalt.com", "vistasand.com"];
        var hasNumber = /\d/;
        if (hasNumber.test(firstName) || firstName === '') {
            errorAlert("First name cannot be blank and cannot contain a number");
            return false;
        }
        if (hasNumber.test(lastName) || lastName === '') {
            errorAlert("Lst name cannot be blank and cannot contain a number");
            return false;
        }
...
// Means it has the rejected domains
        ...
        errorAlert("Must be VPROP, Maalt or Vista Sand domain")
        return false;
    }
