...
/* * *****************************************************************************************************************************************
 * File Name: tl_sampleeditprocess.php
 * Project: Silicore
...
$currentSample = json_decode($db->get('sp_tl_qc_SampleGetByID("' . $sampleId . '");'));
...
{
    $query = 'sp_tl_qc_SampleFinishDtCheck("' . $sampleId . '");';
...
$resultSample = json_decode($db->get('sp_tl_qc_SampleGetByID("' . $sampleId . '");'));

if(checkIfSampleIsComplete($sampleId) == 1) {
    $checkQuery = 'sp_tl_qc_SampleFinishDtCheck("' . $sampleId . '");';
    $finishDtResult = json_decode($db->get($checkQuery))[0]->finish_dt;
    if ($finishDtResult == null) {
        $updateQuery = 'sp_tl_qc_SampleFinishDtUpdate("' . $sampleId . '");';
        $db->insert($updateQuery);
    }
}

echo $resultSample[0]->is_complete;
if($resultSample[0]->is_complete == 1) {
    $db->insert('sp_tl_qc_sample_repeat_unlock("' . $sampleId . '");');
}
...
if($result == 1)
{
    if($isCOA == 1) {
        if($userPermissionsArray['vista']['granbury']['qc'] > 3) {
            ...
            header('Location: ../../Controls/QC/tl_samples.php?view=summary&completionStatus=Incomplete&void=A');
        }
...
else
{
  ...
  //header('Location: ../../Controls/QC/tl_sampleedit.php?sampleId=' . $sampleId);
}
