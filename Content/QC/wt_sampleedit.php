...
if (checkIfSampleIsComplete($sampleId) == 1) {
    ...
    if ($sampleObject->vars['testType'] !== 6) {
        $query = 'sp_wt_qc_RepeatabilitySamplePairsGetByOriginalSample("' . $sampleId . '")';
        $result = json_decode($db->get($query))[0];
        if ($result !== '0') {
            //echo "<script type=\"text/javascript\">window.location = \"../../Controls/QC/wt_sampleview.php?sampleId=" . $sampleId . "\";</script>"; //using JS, because output is already sent in header.php

        }
    } else {
        $query = 'sp_wt_qc_RepeatabilitySamplePairsGetByRepeatedSample("' . $sampleId . '")';
        $result = json_decode($db->get($query))[0];
        if ($result !== '0') {
            //echo "<script type=\"text/javascript\">window.location = \"../../Controls/QC/wt_sampleview.php?sampleId=" . $sampleId . "\";</script>"; //using JS, because output is already sent in header.php
...
<div class="container" style="max-width:85%;">
    ...
    <form id="qcForm" name="qcForm" action="../../Includes/QC/wt_sampleeditprocess.php" onsubmit="return validateForm()"
...
<script>
    ...
    function updateDateTimeSpan() {
        ...
        function confirmSite() {
            ...
            if (siteIDSelected != 60) //DEV NOTE: This site ID is hard coded and should be changed for new mine sites. 10 = West Texas; 50 = Tolar
            {
