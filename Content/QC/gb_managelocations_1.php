<?php
/* * *****************************************************************************************************************************************
 * File Name: gb_managelocations.php
 * Project: Silicore
 ...
 * =========================================================================================================================================
 * 07/10/2018|zthale|KACE:24315 - Initial creation
 * **************************************************************************************************************************************** */
...
function getLocations()
{
  ...
  try
  {
    ...
    $sql = "CALL sp_gb_qc_LocationsNamesGet();"; // Stored procedure method, which gets current locations to display in table.
    $result =  $mySQLConnection->query($sql);
...
<html>
	<head>
		<!--<title>gb_managelocations.php</title>-->
	...
	<body>
    <h1>Manage Locations (Granbury)</h1><br /><br />
      ...
      <!-- Form is adaptable to browser resizing, and each field is required before submission. -->
        ...
        <label for="siteSelect"><strong>Site:</strong></label>
				<select class="form-control" name="siteSelect" id="siteSelect" readonly>
					<?php
                        ...
                        {
                           if($siteObject->vars["id"] == 10) // If the row's ID corresponds to "Granbury" (ID: 10).
                            {
        ...
        <label for="plantSelect"><strong>Plant:</strong></label>
           ...
           <?php
                        ...
                        {
                           if($plantObject->vars["site"] == 10) // If the row's ID corresponds to "Granbury" (ID: 10).
                            {
                                  ...
                                  foreach ($locationObjectArray as $locationObject) 
                                  {
                                    ...
                                    if($locationObject->vars['active_status'] == 0)
                                    {
                                     ...
                                     else if($locationObject->vars['active_status'] == 1)
                                     {
                                      ...
                                      $voidURL = "../../Includes/QC/locationvoid.php?locationID=" . urlencode($locationObject->vars['id']) 
                                              . "&locationOrder=" . urlencode($locationObject->vars['sort_order']) 
                                              . "&locationSiteID=" . urlencode($locationObject->vars['site_id']) 
                                              . "&locationSite=" . urlencode($locationObject->vars['site']) 
                                              . "&locationPlantID=" . urlencode($locationObject->vars['plant_id']) 
                                              . "&locationPlant=" . urlencode($locationObject->vars['plant']) 
                                              . "&locationDescription=" . urlencode($locationObject->vars['description']) 
                                              . "&locationActive=0"
                                              . "&locationSplit=" . urlencode($locationObject->vars['split']) . "&siteCode=gb";            
                                      
                                      echo "<td><a href='" . $editURL . "'><!--Edit--><i class='fa fa-edit' style='font-size:20px;color:green'></i></a></td>";
                                      echo "<td><a href='" . $voidURL . "'><!--Void--><i class='fa fa-trash' style='font-size:20px;color:red' onclick=\"return confirm('Are you sure?')\"></i></a></td></tr>";
                                  }
                              ?>
