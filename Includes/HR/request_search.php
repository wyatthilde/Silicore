...
$db = new Database();
$appEmpCode = filter_input(INPUT_POST, 'appEmpCode');
$allRequestsQuery = 'sp_hr_MedicalRequestByAppEmpCodeGet("' . $appEmpCode . '")';
$allRequestsObj = json_decode($db->get($allRequestsQuery));

$applicantArray = array();
$employeeArray = array();
if(!is_null($allRequestsObj) && $allRequestsObj !== 0) {
    print_r($allRequestsObj);
    $testsArray = Array();
