...
function getLocations()
{
  ...
  try
  {
    ...
    $sql = "CALL sp_wt_qc_LocationsNamesGet();"; // Stored procedure method, which gets current locations to display in table.
    $result =  $mySQLConnection->query($sql);
...
<html>
	<head>
		<!--<title>wt_managelocations.php</title>-->
	...
	<body>
    <h1>Manage Locations (West Texas)</h1><br /><br />
      ...
      <!-- Form is adaptable to browser resizing, and each field is required before submission. -->
        ...
        <label for="siteSelect"><strong>Site:</strong></label>
				<select class="form-control" name="siteSelect" id="siteSelect" readonly>
					<?php
                        ...
                        {
                           if($siteObject->vars["id"] == 60) // If the row's ID corresponds to "West Texas" (ID: 60).
                            {
        ...
        <label for="plantSelect"><strong>Plant:</strong></label>
           ...
           <?php
                        ...
                        {
                           if($plantObject->vars["site"] == 60) // If the row's ID corresponds to "West Texas" (ID: 60).
                            {
